...
The fork keeps Sandboard's WebGPU heightfield and mass-carrying airborne grains. Bright pink and natural sand are the two material palettes, including airborne grains. Natural sand restores the upstream grain-family albedos and neutral sky/ground lighting; bright pink keeps the candy material. The palette is selected by a render uniform. Bright pink is the default; removed light-pink/purple saved preferences fall back to it. The default studio environment replaces the coconut shadow with a neutral 1×1 texture so no palm asset or shadow passes are required. The reset wash uses rose-tinted sky and water scattering.
...
All tools, palettes, installation, and fullscreen controls live in the cog's native modal dialog. The Shapes drawer switches between an eight-shape Cute wheel (including Cat and Dog) and a six-shape Dinosaurs wheel (T. rex, Bronto, Stego, Tricera, cracked egg, and footprint). Only one wheel is visible; arrow-key navigation stays inside it. Selecting a shape or Draw closes the dialog immediately; Water and Fresh sand also close it so the result is visible. Escape, backdrop click, and the close button dismiss the modal; arrow keys move focus around the active wheel. The dialog scrolls on short screens and uses two columns in phone landscape.
